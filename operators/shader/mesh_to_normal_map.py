...
class SHIYUME_OT_MeshToNormalMap(bpy.types.Operator):
    ...
    以网格“清掉自定义法线、全平滑”的原始几何法线作为 base，读取当前自定义法线相对
    该 base 的差值，逐三角形 barycentric 光栅化进 UV，写出切线空间法线贴图。
    与“法线贴图烘焙到网格”互为逆操作；全程在临时副本上取 base，绝不破坏原网格现有的
    自定义法线。"""
    ...
    image_size: bpy.props.IntProperty(
        ...
        description="材质里找不到法线贴图时，新建空白贴图的边长",
    )
    ...
    def execute(self, context):
        ...
        if mesh.uv_layers.active is None:
            self.report({'ERROR'}, "网格缺少 UV，无法烘焙切线空间")
            return {'CANCELLED'}

        if context.mode != 'OBJECT':
            bpy.ops.object.mode_set(mode='OBJECT')

        image = self._resolve_target_image(obj, material)
        ...
        # 5. 三角形 → 像素：barycentric 插值 + 光栅化。
        pixel_matrix, coverage = _rasterize(
            mesh, tangent_space, pixel_x, pixel_y, width, height, self.flip_green
        )

        # 6. 边缘外扩，把已覆盖像素往 UV 岛外推，消除接缝。
        if self.margin > 0:
            _dilate(pixel_matrix, coverage, self.margin)

        # 7. 写回贴图（法线贴图必须是 Non-Color）。
        image.colorspace_settings.name = 'Non-Color'
        bake_common.write_image_pixels(image, pixel_matrix)

        self.report(
            {'INFO'},
            f"已把 {len(mesh.loops)} 条法线烘焙到 {image.name} ({width}x{height})",
        )
        return {'FINISHED'}

    def _resolve_target_image(self, obj, material):
        """取烘焙目标贴图：优先用材质里已有的法线贴图，没有就新建一张并挂上节点。"""
        ...
        new_node = material.node_tree.nodes.new(type='ShaderNodeTexImage')
        new_node.image = image
        new_node.label = "Normal (Shiyume Bake)"
        material.node_tree.nodes.active = new_node
        self.report({'WARNING'}, f"材质里没有法线贴图，已新建 {image.name}")
        return image
...
def _rasterize(mesh, tangent_space, pixel_x, pixel_y, width, height, flip_green):
    """逐三角形把切线空间法线 barycentric 光栅化进像素矩阵。

    返回 (pixel_matrix[h,w,4], coverage[h,w])；未覆盖区域预填平面法线 (0.5,0.5,1.0)。
